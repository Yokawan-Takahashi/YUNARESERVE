...
class BookingFlowTest extends TestCase
{
    ...
    {
        $slug = $this->slug();
        $response = $this->get("/{$slug}/events/{$this->event->id}/book/{$this->slot->id}");
    ...
    {
        $slug = $this->slug();
        $response = $this->post("/{$slug}/events/{$this->event->id}/book/{$this->slot->id}", $this->validData());
    ...
    {
        $slug = $this->slug();
        $this->post("/{$slug}/events/{$this->event->id}/book/{$this->slot->id}", $this->validData());
    ...
    public function test_slot_becomes_full_when_capacity_reached(): void
    {
        ...
        $this->post("/{$slug}/events/{$this->event->id}/book/{$fullSlot->id}", $this->validData());
    ...
    public function test_cannot_book_full_slot(): void
    {
        ...
        $response = $this->post("/{$slug}/events/{$this->event->id}/book/{$fullSlot->id}", $this->validData());
    ...
    {
        $slug = $this->slug();
        $response = $this->post("/{$slug}/events/{$this->event->id}/book/{$this->slot->id}",
    ...
    public function test_done_page_shows_reservation_code(): void
    {
        ...
        $response = $this->get("/{$slug}/done/{$reservation->code}");
        $response->assertStatus(200);
    ...
    public function test_cannot_book_draft_event(): void
    {
        ...
        $draftSlot = Slot::create([
            'event_id' => $draftEvent->id,
            'date' => '2026-09-01', 'start_time' => '10:00', 'capacity' => 5,
        ]);

        $response = $this->post("/{$slug}/events/{$draftEvent->id}/book/{$draftSlot->id}", $this->validData());
